...
class PolicyPublished:
    ...
    def _validate_policy(self):
        ...
        if self.p not in valid_policies:
            raise ValueError(f"Invalid policy '{self.p}'. Must be one of: {valid_policies}")
        if self.sp is not None and self.sp not in valid_policies:
            raise ValueError(f"Invalid subdomain policy '{self.sp}'. Must be one of: {valid_policies}")
    ...
    def _validate_alignment_modes(self):
        """Validate alignment modes."""
        valid_modes = {'r', 's', None}
        if self.adkim not in valid_modes:
            raise ValueError(f"Invalid DKIM alignment mode '{self.adkim}'. Must be 'r' or 's'")
        if self.aspf not in valid_modes:
            raise ValueError(f"Invalid SPF alignment mode '{self.aspf}'. Must be 'r' or 's'")
...
class PolicyEvaluated:
    ...
    def _validate_disposition(self):
        ...
        if self.disposition not in valid_dispositions:
            raise ValueError(f"Invalid disposition '{self.disposition}'. Must be one of: {valid_dispositions}")
    
    def _validate_auth_results(self):
        """Validate authentication results."""
        valid_results = {'pass', 'fail'}
        if self.dkim not in valid_results:
            raise ValueError(f"Invalid DKIM result '{self.dkim}'. Must be 'pass' or 'fail'")
        if self.spf not in valid_results:
            raise ValueError(f"Invalid SPF result '{self.spf}'. Must be 'pass' or 'fail'")
...
class DMARCRecord:
    ...
    def _validate_header_from(self):
...
class DMARCReport:
    ...
    def get_alignment_rate(self) -> float:
        ...
        aligned_messages = sum(
            record.count for record in self.records 
            if record.is_dmarc_aligned()
        )
    ...
    def to_dict(self) -> Dict[str, Any]:
        ...
        return convert_datetimes(report_dict)
    ...
    def from_dict(cls, data: Dict[str, Any]) -> 'DMARCReport':
        ...
        try:
            ...
            def convert_datetimes(data):
                if isinstance(data, dict):
                    ...
                    for k, v in data.items():
                        if k in ['date_range_begin', 'date_range_end'] and isinstance(v, str):
            ...
            for record_data in converted_data['records']:
                ...
                dkim_results = [AuthResult(**auth_data) for auth_data in record_data['dkim_results']]
                spf_results = [AuthResult(**auth_data) for auth_data in record_data['spf_results']]
