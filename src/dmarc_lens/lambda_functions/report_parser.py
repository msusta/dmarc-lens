...
from ..utils.email_utils import (
    parse_email_from_string, extract_dmarc_reports, EmailParsingError,
    AttachmentExtractionError, get_email_metadata, validate_email_structure
)
from ..utils.xml_utils import (
    parse_xml_string, validate_dmarc_xml_structure, XMLValidationError,
    XMLParsingError, extract_xml_text, extract_xml_int, extract_xml_timestamp
)
    ...
    try:
        ...
        for filename, xml_content in dmarc_reports:
            try:
                ...
                try:
                    store_failed_report(filename, xml_content, str(e), email_metadata, object_key)
...
def parse_dmarc_report_xml(root) -> DMARCReport:
    """
...
def parse_dmarc_record_xml(record_elem) -> DMARCRecord:
    """
    ...
    policy_evaluated = PolicyEvaluated(
        ...
        reason=None  # TODO: Parse reason elements if present
    )
...
def store_dmarc_report(report: DMARCReport, email_metadata: Dict[str, Any], 
                      s3_key: str, filename: str) -> None:
    """
    ...
    try:
        ...
        logger.info(f"Stored {len(report.records)} records for report {report.metadata.report_id}")
...
def store_failed_report(filename: str, xml_content: str, error_message: str,
                       email_metadata: Dict[str, Any], s3_key: str) -> None:
    """
