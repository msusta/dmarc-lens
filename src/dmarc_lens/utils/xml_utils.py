...
def validate_dmarc_xml_structure(root: ET.Element) -> bool:
    ...
    try:
        ...
        if root.tag != 'feedback':
            raise XMLValidationError(f"Invalid root element: {root.tag}. Expected 'feedback'")
        ...
        if element == 'date_range':
            ...
            for date_element in schema['date_range_elements']:
                if date_range.find(date_element) is None:
                    raise XMLValidationError(f"Missing date_range/{date_element} element")
                
                # Validate timestamp format
                timestamp_text = date_range.find(date_element).text
                if not timestamp_text or not timestamp_text.isdigit():
                    raise XMLValidationError(f"Invalid timestamp in date_range/{date_element}")
        else:
...
def _validate_policy_published(root: ET.Element) -> None:
    ...
    for element in schema['required_elements']:
        ...
        if element_node is None or not element_node.text:
            raise XMLValidationError(f"Missing or empty policy_published/{element} element")
    ...
    for element in schema.get('optional_elements', []):
        ...
        if element_node is not None and element_node.text:
            if element in ['sp'] and element_node.text not in schema['valid_policies']:
                raise XMLValidationError(f"Invalid subdomain policy value: {element_node.text}")
            elif element in ['adkim', 'aspf'] and element_node.text not in schema['valid_alignment']:
                raise XMLValidationError(f"Invalid alignment mode: {element_node.text}")
            elif element == 'pct':
                try:
                    pct_value = int(element_node.text)
                    if not 0 <= pct_value <= 100:
                        raise XMLValidationError(f"Invalid percentage value: {pct_value}")
                except ValueError:
                    raise XMLValidationError(f"Invalid percentage format: {element_node.text}")
    ...
    for i, record in enumerate(records):
            ...
            if row_element == 'policy_evaluated':
                ...
                for pe_element in schema['policy_evaluated_elements']:
                    ...
                    if pe_node is None or not pe_node.text:
                        raise XMLValidationError(f"Missing policy_evaluated/{pe_element} in record {i}")
            else:
                ...
                if row_element == 'source_ip':
                    if not _is_valid_ip(row_node.text):
                        raise XMLValidationError(f"Invalid IP address in record {i}: {row_node.text}")
                elif row_element == 'count':
                    try:
                        count_value = int(row_node.text)
                        if count_value <= 0:
                            raise XMLValidationError(f"Invalid count value in record {i}: {count_value}")
                    except ValueError:
                        raise XMLValidationError(f"Invalid count format in record {i}: {row_node.text}")
        ...
        for id_element in schema['identifiers_elements']:
            ...
            if id_node is None or not id_node.text:
                raise XMLValidationError(f"Missing identifiers/{id_element} in record {i}")
...
def get_xml_statistics(root: ET.Element) -> Dict[str, Any]:
    ...
    try:
        stats = {
            ...
            'namespaces': list(set(elem.tag.split('}')[0][1:] 
