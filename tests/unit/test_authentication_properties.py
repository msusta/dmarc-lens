...
def valid_jwt_claims(draw):
    ...
    return {
        ...
        "iat": current_time
        - draw(st.integers(min_value=0, max_value=3600)),  # Issued in the past
        "exp": current_time
        + draw(st.integers(min_value=60, max_value=3600)),  # Expires in the future
        "nbf": current_time
        - draw(st.integers(min_value=0, max_value=60)),  # Valid from past/now
...
def invalid_jwt_claims(draw):
    ...
    invalidity_type = draw(
        st.sampled_from(["invalid_token_use", "future_nbf", "missing_sub"])
    )
...
def api_gateway_auth_event(draw):
    ...
    method_path = draw(
        st.sampled_from([("POST", "/auth/validate"), ("GET", "/health")])
    )
