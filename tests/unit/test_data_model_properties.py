...
def valid_email_addresses(draw):
    ...
    domain_part = draw(
        st.sampled_from(["example.com", "test.org", "sample.net", "demo.co.uk"])
    )
...
def report_metadata_strategy(draw):
    ...
    end_timestamp = begin_time.timestamp() + draw(
        st.integers(min_value=3600, max_value=86400)
    )
    end_time = datetime.fromtimestamp(end_timestamp)

    return ReportMetadata(
        org_name=draw(
            st.sampled_from(["Example Corp", "Test Org", "Sample Inc", "Demo Company"])
        ),
...
def policy_published_strategy(draw):
    ...
    return PolicyPublished(
        ...
        sp=draw(
            st.one_of(st.none(), st.sampled_from(["none", "quarantine", "reject"]))
        ),
    ...
    return PolicyEvaluated(
        ...
        reason=draw(
            st.one_of(
                st.none(),
                st.lists(
                    st.sampled_from(["forwarded", "sampled_out", "trusted_forwarder"]),
                    min_size=1,
                    max_size=3,
                ),
            )
...
def auth_result_strategy(draw):
    ...
    return AuthResult(
        ...
        result=draw(
            st.sampled_from(
                ["pass", "fail", "neutral", "policy", "temperror", "permerror"]
            )
        ),
        selector=draw(
            st.one_of(
    ...
    **Validates: Requirements 2.1, 2.2, 2.3, 2.5**
    ...
    def test_dmarc_report_dict_round_trip(self, dmarc_report):
        ...
        **Feature: dmarc-analysis, Property 3: DMARC Report Parsing Round-Trip**
        ...
        assert (
            reconstructed_report.metadata.report_id == dmarc_report.metadata.report_id
        )
        assert (
            reconstructed_report.metadata.date_range_begin
            == dmarc_report.metadata.date_range_begin
        )
        assert (
            reconstructed_report.metadata.date_range_end
            == dmarc_report.metadata.date_range_end
        )
        assert (
            reconstructed_report.metadata.extra_contact_info
            == dmarc_report.metadata.extra_contact_info
        )

        # Verify policy information is preserved
        assert (
            reconstructed_report.policy_published.domain
            == dmarc_report.policy_published.domain
        )
        assert (
            reconstructed_report.policy_published.p == dmarc_report.policy_published.p
        )
        assert (
            reconstructed_report.policy_published.sp == dmarc_report.policy_published.sp
        )
        assert (
            reconstructed_report.policy_published.pct
            == dmarc_report.policy_published.pct
        )
        assert (
            reconstructed_report.policy_published.adkim
            == dmarc_report.policy_published.adkim
        )
        assert (
            reconstructed_report.policy_published.aspf
            == dmarc_report.policy_published.aspf
        )
        ...
        for orig_record, recon_record in zip(
            dmarc_report.records, reconstructed_report.records
        ):
            ...
            assert (
                recon_record.policy_evaluated.disposition
                == orig_record.policy_evaluated.disposition
            )
            assert (
                recon_record.policy_evaluated.dkim == orig_record.policy_evaluated.dkim
            )
            assert recon_record.policy_evaluated.spf == orig_record.policy_evaluated.spf
            assert (
                recon_record.policy_evaluated.reason
                == orig_record.policy_evaluated.reason
            )
            ...
            for orig_dkim, recon_dkim in zip(
                orig_record.dkim_results, recon_record.dkim_results
            ):
            ...
            for orig_spf, recon_spf in zip(
                orig_record.spf_results, recon_record.spf_results
            ):
    ...
    def test_dmarc_report_json_round_trip(self, dmarc_report):
        ...
        **Feature: dmarc-analysis, Property 3: DMARC Report Parsing Round-Trip**
        ...
        assert (
            reconstructed_report.get_total_messages()
            == dmarc_report.get_total_messages()
        )
        assert (
            abs(
                reconstructed_report.get_alignment_rate()
                - dmarc_report.get_alignment_rate()
            )
            < 0.01
        )
        assert set(reconstructed_report.get_source_ips()) == set(
            dmarc_report.get_source_ips()
        )
    ...
    def test_serialization_preserves_business_logic(self, dmarc_report):
        ...
        **Feature: dmarc-analysis, Property 3: DMARC Report Parsing Round-Trip**
        ...
        assert (
            abs(reconstructed_report.get_alignment_rate() - original_alignment_rate)
            < 0.01
        )
        ...
        assert (
            reconstructed_summary["total_messages"]
            == original_summary["total_messages"]
        )
        assert (
            abs(
                reconstructed_summary["alignment_rate"]
                - original_summary["alignment_rate"]
            )
            < 0.01
        )
        assert (
            reconstructed_summary["unique_sources"]
            == original_summary["unique_sources"]
        )
        assert reconstructed_summary["domain"] == original_summary["domain"]

        # Verify disposition breakdown is preserved
        assert (
            reconstructed_summary["disposition_breakdown"]
            == original_summary["disposition_breakdown"]
        )
        ...
        for orig_record, recon_record in zip(
            ...
            assert (
                recon_auth_summary["dmarc_aligned"]
                == orig_auth_summary["dmarc_aligned"]
            )
            ...
            assert (
                recon_auth_summary["message_count"]
                == orig_auth_summary["message_count"]
            )
    ...
    def test_invalid_json_handling(self, invalid_json):
        ...
        **Feature: dmarc-analysis, Property 3: DMARC Report Parsing Round-Trip**
        ...
        assume(
            not (
                invalid_json.strip().startswith("-")
                and invalid_json.strip()[1:].isdigit()
            )
        )
